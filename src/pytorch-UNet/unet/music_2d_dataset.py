...
import numpy as np
import argparse
from  unet.music_2d_labels import MUSIC_2D_LABELS
import torch
import albumentations as A
from albumentations.pytorch import ToTensorV2
import torchio as tio

from unet.data_utils import dimensionality_reduction
...
class MUSIC2DDataset(Dataset):
    ...
    def _get_classes(self, mask):
        if not torch.is_tensor(mask):
            mask = torch.stack(mask)
            list_classes = []
            for i in range(mask.shape[0]):
                  classes = torch.zeros((len(self.classes)))
                  uniques = torch.unique(mask[i].argmax(0))
                  classes[uniques] = 1
                  list_classes.append(classes)
            classes = torch.stack(list_classes)
        else:         
            classes = torch.zeros((len(self.classes)))
            uniques = torch.unique(mask.argmax(0))
            classes[uniques] = 1
        return classes

    def __getitem__(self, index):
        image = self._get_image(index=index)
        mask = self._get_mask(index=index) 
        if self.transform:
            image, mask = self.transform(image, mask)
        classes = self._get_classes(mask)
        return {"image": image, "mask": mask, "classes":classes}
    ...
    def _load_data(self):
        for path in os.listdir(self.path2d):
            ...
            mask_file = h5py.File(os.path.join(self.path2d, path, "manualSegmentation",
        ...
        if self.full_dataset and (self.partition=="train"):
            for path in os.listdir(self.path3d):
                ...
                # between the number of slices and the number of masks
                ...
                mask_file = h5py.File(os.path.join(self.path3d, path, 
                                            "manualSegmentation", 
                                            "manualSegmentation_global.h5"))
                # Open reconstructions
                reconstruction_file = h5py.File(os.path.join(data_path, "reconstruction.h5"),"r")
...
if __name__ == "__main__":
    ...
    print(len(dataset[:]["mask"]))
    print(dataset[0]["image"].shape)
    print(dataset[0]["mask"].shape)
    #print(np.argmax(dataset[0]["mask"],0).unique())
    dataset_list = []
    for data in (dataset):
        subject = tio.Subject(
            image=tio.ScalarImage(tensor=data["image"].unsqueeze(0)),
            mask=tio.LabelMap(tensor=torch.argmax(data["mask"],0).unsqueeze(0).repeat(1,10,1,1)),
        )
    ...
    for patches_batch in patches_loader:
        ...
        targets = patches_batch['mask'][tio.DATA]  # key 'brain' is in subject
        print(targets.shape)
