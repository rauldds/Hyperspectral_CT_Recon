...
class Model:
    ...
    def fit_epoch(self, dataset, n_batch=1, shuffle=False, use_one_hot=True):
        """
        ...
        for batch_idx, data in enumerate(DataLoader(dataset, batch_size=n_batch, shuffle=shuffle)):
    ...
    def val_epoch(self, dataset, n_batch=1, metric_list=MetricList({}), use_one_hot=True):
        """
        ...
        for batch_idx, data in enumerate(DataLoader(dataset, batch_size=n_batch)):
    ...
    def fit_dataset(self, dataset: ImageToImage2D, n_epochs: int, n_batch: int = 1, shuffle: bool = False,
                    ...
                    verbose: bool = False, use_one_hot=True):
        ...
        for epoch_idx in range(1, n_epochs + 1):
            # doing the epoch
            train_logs = self.fit_epoch(dataset, n_batch=n_batch, shuffle=shuffle, use_one_hot=use_one_hot)

            if self.scheduler is not None:
                self.scheduler.step(train_logs['train_loss'])

            if val_dataset is not None:
                val_logs = self.val_epoch(val_dataset, n_batch=n_batch, metric_list=metric_list, use_one_hot=use_one_hot)
            ...
            tb.add_scalar("train_loss", train_logs["train_loss"], epoch_idx)
            tb.add_scalar("val_loss", val_logs["val_loss"], epoch_idx)
            tb.add_scalar("val_iou", val_logs["jaccard"], epoch_idx)
    ...
    def predict_dataset(self, dataset, export_path, patch_size=20):
        """
        ...
        for batch_idx, data in enumerate(DataLoader(dataset, batch_size=1)):
