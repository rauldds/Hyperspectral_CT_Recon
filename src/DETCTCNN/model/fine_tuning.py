...
from src.DETCTCNN.model.utils import calculate_min_max, class_weights, class_weights_sklearn, image_from_segmentation, \
    plot_segmentation, calculate_data_statistics, standardize, normalize
...
def main(hparams):
    ...
    transform = music_2d_dataset.JointTransform2D(crop=(hparams.patch_size, hparams.patch_size), p_flip=0.5,
                                                  color_jitter_params=None, long_mask=True, erosion=hparams.erosion,
                                                  p_random_affine=0.3)
    valid_transform = music_2d_dataset.JointTransform2D(crop=(96, 96), p_flip=0.5, color_jitter_params=None,
                                                        long_mask=True, erosion=hparams.erosion)
    ...
    print(f"\n class weights after zeroing out non relevant classes {dice_weights}")
    ...
    checkpoint = torch.load('/Users/davidg/Downloads/DETECTCNN_EXPs/2/model_focal.pt',
    ...
    tb = SummaryWriter(
        f'runs/{hparams.experiment_name}/patch_size_{hparams.patch_size}_{datetime.datetime.now().strftime("%b%d_%H-%M-%S")}')
    ...
    if hparams.loss == "ce":
        ...
        # Use Weighted Dice Loss
        loss_criterion = FocalLoss(gamma=hparams.gamma, alpha=dice_weights, reduction=hparams.dice_reduc,
                                   ignore_index=hparams.ignore_index).to(device)
    ...
    for epoch in range(hparams.epochs):  # loop over the dataset multiple times
        ...
        if epoch % hparams.print_every == (hparams.print_every - 1):
            ...
            print(
                f'[epoch: {epoch:03d}/iteration: {i :03d}] train_loss: {running_loss / hparams.print_every :.6f}, train_acc: {train_accuracy:.2f}%, train_IOU: {train_iou:.2f}%')
            print(
                f'[epoch: {epoch:03d}/iteration: {i :03d}] train IOU per class in batch: {["{0:0.2f}".format(j) for j in train_iou_per_class]}')
        ...
        if epoch % hparams.validate_every == (hparams.validate_every - 1):
            ...
            for val_data in val_loader:
                ...
                val_iou_per_class_cur, val_iou_cur = mIoU_score(val_pred.cpu().argmax(1), val_y.cpu(),
            ...
            print(
                f'[INFO-Validation][epoch: {epoch:03d}/iteration: {i :03d}] validation_loss: {val_loss:.6f}, validation_acc: {val_acc:.2f}%, validation_IOU: {val_iou:.2f}%')
            print(
                f'[INFO-Validation][epoch: {epoch:03d}/iteration: {i :03d}] validation IOU per class in batch: {["{0:0.2f}".format(j) for j in val_iou_per_class]}')
...
if __name__ == "__main__":
    ...
    parser.add_argument("-e", "--epochs", type=int, default=2000, help="Number of maximum training epochs")
    parser.add_argument("-bs", "--batch_size", type=int, default=256, help="Batch size")
    parser.add_argument("-bsv", "--batch_size_val", type=int, default=64, help="Batch size")
    ...
    parser.add_argument("-dim_red", "--dim_red", choices=['none', 'pca', 'merge'], default="none",
                        help="Use dimensionality reduction")
    parser.add_argument("-no_dim_red", "--no_dim_red", type=int, default=10,
                        help="Target no. dimensions for dim reduction")
    parser.add_argument("-sample_strategy", "--sample_strategy", choices=['grid', 'label'], default="label",
                        help="Type of sampler to use for patches")
    ...
    parser.add_argument("-mn", "--model_name", type=str, default="model_focal_fine-tuning_0.0005_lr",
                        help="apply erosion as augmention")
    parser.add_argument("-ii", "--ignore_index", type=int, default=MUSIC_2D_LABELS["container"],
                        help="Which Index class to ignore")
