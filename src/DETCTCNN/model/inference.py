...
def main(args):
    ...
    model = get_model(input_channels=INPUT_CHANNELS[args.spectrum], n_labels=args.n_labels,use_bn=True, basic_out_channel=16, depth=3, dropout=0)
    checkpoint = torch.load("model_all_best.pt", map_location=torch.device("cuda" if torch.cuda.is_available() else "cpu"))
