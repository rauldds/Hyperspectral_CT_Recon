...
def main(hparams):
    ...
    transform = music_2d_dataset.JointTransform2D(crop=(hparams.patch_size, hparams.patch_size), p_flip=0.5, color_jitter_params=None, long_mask=True, p_random_affine=0.20)
    ...
    model = get_model(input_channels=energy_levels, n_labels=hparams.n_labels, use_bn=True, basic_out_channel=64, depth=2, dropout=0.7)
    ...
    tb = SummaryWriter(f'runs/patch_size_{hparams.patch_size}_{datetime.datetime.now().strftime("%b%d_%H-%M-%S")}')
